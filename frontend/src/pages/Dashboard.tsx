...
const Dashboard: React.FC = () => {
  ...
  const { data: agendamentosHoje, isLoading: loadingAgendamentos } = useQuery<AgendamentoDTO[]>({
    ...
    queryFn: async () => {
      ...
      const response = await apiService.get('/api/agendamentos/por-data', { params: { data: hoje } });
      return Array.isArray(response.data) ? response.data : response.data?.data || [];
    },
    refetchInterval: 30000,
  });

  const agendamentosHojeArray: AgendamentoDTO[] = Array.isArray(agendamentosHoje) ? agendamentosHoje : [];
  ...
  const agendados = agendamentosHojeArray.filter(a => a.status === 'AGENDADO').length || 0;
  const confirmados = agendamentosHojeArray.filter(a => a.status === 'CONFIRMADO').length || 0;
  const realizados = agendamentosHojeArray.filter(a => a.status === 'REALIZADO').length || 0;
  const cancelados = agendamentosHojeArray.filter(a => a.status === 'CANCELADO').length || 0;
  const totalHoje = agendamentosHojeArray.length || 0;
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            ) : agendamentosHojeArray.length > 0 ? (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {agendamentosHojeArray.sort((a, b) => new Date(a.dataHora).getTime() - new Date(b.dataHora).getTime())
                        .map((agendamento) => (
